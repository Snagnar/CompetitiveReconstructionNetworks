...
class CommandLineLogger(ProgressBarBase):
    ...
    def on_train_batch_end(
        ...
        if self._average_epoch_time() > 0:
            epoch_total_est = self._average_epoch_time()
        print("epochs left:", full_epochs_left, "est epoch time:", epoch_total_est)
        ...
        avg_steps_per_second = trainer.global_step / total_time
        ...
        time_per_step = f"Steps per Second: {avg_steps_per_second:.3f}"
        self.log_batch(f"{epoch_info} - {batch_info} - {metrics} - {eta_info} - {time_per_step}")
        ...
        for key, value in metrics_dict.items():
            ...
            try:
                ...
                if math.isnan(f_value):
                    continue
                if key:
                    metric_list.append(f"{key}={f_value:2.4f}")
            except ValueError:
                if key:
    ...
    def on_train_epoch_start(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        if self._train_start_time is None or self._train_start_time == 0.0:
    ...
    def on_train_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        print("on epoch end, last epoch times:", self._last_epoch_times)
